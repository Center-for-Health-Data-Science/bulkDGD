...
def plot_residuals_hist(df_res,
    ...
    df_res = \
        df_res[\
            [col for col in df_res.columns if col.startswith("ENSG")]]
    ...
    # Generate the histogram.
    n, _, patches = ax.hist(x = residuals,
                            **config_hist)
...
def plot_multiple_residuals_hist(dfs_res,
    ...
    for num_output, dfs_chunk in enumerate(dfs_chunks):
        ...
        # For each data frame
        for num_df, (df_res, ax) in \
            enumerate(zip(dfs_chunk,
                          axes.flat[:len(dfs_chunk)])):
            ...
            if config_cbar:
                ...
                # Color the histogram bars based on the normalized
                # height.
                for count, patch in zip(n, patches):
                    color = cmap(norm(count))
                    patch.set_facecolor(color)

                # Create a 'ScalarMappable' object to use for the
                # colorbar.
                sm = ScalarMappable(cmap = cmap,
                                    norm = norm)
