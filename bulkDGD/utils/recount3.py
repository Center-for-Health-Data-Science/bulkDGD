...
#    recount3.py
#
#    Utility functions to interact with the Recount3 platform.
#
...
def check_samples_category(samples_category,
    ...
    supported_categories = \
        misc.get_list(SUPPORTED_CATEGORIES_FILE[project_name])
...
def get_metadata_fields(project_name):
    ...
    # Open the file
    with open(METADATA_FIELDS_FILE[project_name], "r") as f:
...
def get_gene_sums(samples_category,
    ...
    f_gene_sums_name = \
        RECOUNT3_GENE_SUMS_FILE.format(project_name,
                                       samples_category)
    ...
    gene_sums_url = \
        RECOUNT3_GENE_SUMS_URL.format(project_name,
                                      samples_category[-2:],
                                      samples_category,
                                      project_name,
                                      samples_category)
...
def get_metadata(samples_category,
    ...
    f_metadata_name = \
        RECOUNT3_METADATA_FILE.format(project_name,
                                      samples_category)
    ...
    metadata_url = \
        RECOUNT3_METADATA_URL.format(project_name,
                                     samples_category[-2:],
                                     samples_category,
...
def merge_gene_sums_and_metadata(df_gene_sums,
    ...
    if set(df_metadata.columns.tolist() + ["external_id"]) \
        ...
        # an exception
        errstr = \
            f"The fields found in the metadata downloaded " \
            f"from Recount3 do not correspond to the ones " \
            f"in the '{METADATA_FIELDS_FILE[project_name]}' " \
            f"file. Please update the " \
            f"'{METADATA_FIELDS_FILE[project_name]}' file " \
            f"and rerun."
        raise ValueError(errstr)
